...
describe IgnoreColumns do
  ...
  it "should remove the columns from the class" do
    expect(TestModel.column_names.sort).to eq ["id", "name"]
  ...
  it "should remove the columns from the attributes hash" do
    expect(TestModel.new.attributes.keys.sort).to eq ["id", "name"]
    expect(Thing.new.attributes.keys.sort).to eq ["id", "test_model_id", "value"]
  end

  it "should remove the columns from the attribute names" do
    expect(TestModel.new.attribute_names.sort).to eq ["id", "name"]
    expect(Thing.new.attribute_names.sort).to eq ["id", "test_model_id", "value"]
  end
  ...
  it "should not override existing methods with ignored column accessors" do
    model = TestModel.new
    expect(model.attributes).to eq({"id" => nil, "name" => nil})
  ...
  it "should not affect selects" do
    ...
    expect(model.attributes["legacy"]).to be_nil 
    expect(model.attributes["attributes"]).to be_nil 
  end
  ...
  it "should not affect updates" do
    ...
    expect(results).to eq({"name"=>"test2", "legacy" => 1, "attributes" => "woo"})
  end
  ...
  it "should work with associations" do
    ...
    expect(model.things.first.value).to eq 10
    expect(thing.test_model.name).to eq "test"
  end
  ...
  it "should work with magic timestamp columns" do
    ...
    expect(results).to eq({"id" => 1, "value" => 10, "test_model_id" => 1, "updated_at" => nil, "created_at" => nil})
  end
end
